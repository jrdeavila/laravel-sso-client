...
- **Ruta** `POST /sso/logout` (named `sso.logout`) — invalida sesión y cierra la pestaña o redirige al lanzador
- **Ruta** `POST /sso/verify-secret` (named `sso.verify-secret`) — sin CSRF, llamada server-to-server desde el lanzador
